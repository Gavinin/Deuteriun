...
public class SysFilesController {
    ...
    public Result fileList(HttpServletRequest request, @RequestBody SysFiles sysFiles) {
    ...
    public Result getFile(HttpServletRequest request, @RequestBody SysFiles sysFiles) {
        List<SysFiles> sysFiles1 = sysFilesService.mixList(sysFiles);
        if (sysFiles1.size()>0){
            return Result.success(sysFiles1);
        }
